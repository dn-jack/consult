...
import com.consult.util.ReadExcel;
import com.consult.util.SpringContextHolder;
...
public class UploadExcel extends HttpServlet {
	...
	Logger logger = Logger.getLogger(UploadExcel.class);
		...
		try {
			...
			for (Iterator iter = fileItems.iterator(); iter.hasNext();) {
				...
				if (!item.isFormField()) {
					...
					String imageName = new Date().getTime() + ".xls";
					File file = new File(datePath, imageName);
					logger.info("路径========" + file.getAbsolutePath());
					item.write(file);

					ReadExcel readExcel = (ReadExcel) SpringContextHolder
							.getWebApplicationContext().getBean("readExcel");
					
					List<Map<String, String>> reList = readExcel.readXls(file.getAbsolutePath());
					
					CommonService commonService = (CommonService) SpringContextHolder
							.getWebApplicationContext().getBean("commonServiceImpl");

//					JSONObject reJo = new JSONObject();
//					reJo.put("respCode", "0000");
//					reJo.put("respMsg", "excel upload succsess");
//					reJo.put(
//							"excelAddr",
//							request.getScheme() + "://"
...
//									+ temp_str + "/" + imageName);
					out.print(commonService.saveConstract(reList));
				}
			}
		} catch (Exception e) {
